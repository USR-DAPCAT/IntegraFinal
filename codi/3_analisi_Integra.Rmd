...
dt_temp_percent%>% 
  ggplot(., aes(x=hba1cNGSP_7c, group=interaction( grupIntervencio), fill=grupIntervencio)) + 
   geom_bar(aes(y=..prop..), stat="count", position=position_dodge()) +
    geom_text(aes(label=scales::percent(round(..prop..,4)), y=..prop..), stat="count", vjust=-.5, position=position_dodge(.9)) +
...
```{r Resultats4, include=T, results='asis'}
#cluster:[]
...
### 3.2 t-test Diferencia de mitjanes entre Basal i Final, dins de cada grup

```{r Resultats5, include=T, results='asis'}
...
### 3.4 Regressio Lineal Mitjanes de Diferència hba1c
...
### 3.5 Regressio Lineal Mitjanes de Diferència hba1c amb Cluster.
