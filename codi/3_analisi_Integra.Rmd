...
**Lectures , Agregacions i Part Descriptiva**
...
&check; RevisiÓ i depuraciÓ errors.   <br/>
...
table1::table1(~ hba1cNGSP | grupIntervencio*tipus,
...
taula_coef<-exp(confint(mod_lineal)) %>% as_tibble()

taula_coef %>% bind_cols(coef) %>% 
  bind_cols((coef(mod_lineal))) %>% 
  select(var="...3",Coef="...4",IC95_linf="2.5 %",IC95_lsup="97.5 %") %>%
  kable(caption="Regressio Lineal Mitjanes de Diferència hba1c",digits = 2) %>% 
...
taula_coef<-exp(confint(mod_lineal_cluster)) %>% as_tibble()

taula_coef %>% bind_cols(coef) %>% 
  bind_cols((coef(mod_lineal_cluster))) %>% 
  select(var="...3",Coef="...4",IC95_linf="2.5 %",IC95_lsup="97.5 %") %>%
  kable(caption="Regressio Lineal Mitjanes de Diferència hba1c amb Cluster",digits = 2) %>% 
  kableExtra::kable_classic_2()
