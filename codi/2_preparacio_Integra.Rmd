...
TAULA_PLANA<-
  TAULA_PLANA %>% comptar_valors(pp,"Si") %>% 
  mutate(INSULIN=if_else(num_valors>=1,"Si","No"))

## Generacio de INSULINA + NIAD
TAULA_PLANA<-TAULA_PLANA %>% mutate(NIAD_INSULIN=if_else(INSULIN=="Si" & NIAD=="Si","Si","No"))

## Generacio de Tx Dieta (No INSULINA + No NIAD)
TAULA_PLANA<-TAULA_PLANA %>% mutate(LIFE_STYLE=if_else(INSULIN=="No" & NIAD=="No","Si","No"))
...
table(TAULA_PLANA$INSULIN,TAULA_PLANA$NIAD)
table(TAULA_PLANA$NIAD)
table(TAULA_PLANA$kk)
